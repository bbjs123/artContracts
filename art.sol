...
contract Art is Ownable,ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;
  address public usdtAddress = address(0x55d398326f99059fF775485246999027B3197955);//usdt代币地址
  address public receiveAddress = address(0x6CeD42465C8CB4D7Bf3Be3B3bF807B6FC617d46a);//收币地址 
  address public tokenAddress = address(0xe97364f13Fc8f40B0ab372a51b645a648f091483);//pigtoken币合约地址
  uint256 public price = 150;//jiage
  uint256 public totalAmount = 2100 * 10**18 ;
  uint256 public minnum = 1 * 10 ** 18;//zui xiao shu liang
  uint256 public maxnum = 10 * 10 ** 18 ;//zui da shu liang
  uint256 public recRatio = 10;
  uint256 public haveBuyAmount = 0;
  uint256 public openGetTime = 0; //开发领取时间
  mapping(address => uint256) public userCount;
  mapping(address => uint256) public introRewards;
  mapping(address => address) public parents;
  
  
  event InvestLog(address indexed user,uint256 amount,uint256 time,address indexed intro);
  event withdrawTokenLog(address indexed user,uint256 amount,uint256 time);
  ...
  */ 
  function invest (uint256 amount, address intro) public  nonReentrant {
    ...
    if(parents[msg.sender] != address(0)){
        introRewards[parents[msg.sender]] = introRewards[parents[msg.sender]].add(amount.mul(recRatio).div(100));
  ...
  function withdrawReward()public nonReentrant{
      ...
      introRewards[msg.sender] = 0;
      IERC20(tokenAddress).safeTransfer(msg.sender, amount);
      emit withdrawRewardLog(msg.sender,amount,block.timestamp);
  }
